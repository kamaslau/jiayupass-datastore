...
// Local
import { DDB, RDB } from './engines/index.js'
 ...
 * 初始化数据库引擎
 *
 * @param engine {string} 数据库引擎名称
 */
const initDB = async (
  engine: string | undefined = 'RDB'
): Promise<any | null> => {
  const engines = ['DDB', 'RDB'] // TODO 从 /src/engines 目录下获取可用引擎，并初步验证各自可用性（connect、disconnect方法的存在性）

  console.log('initDB: ', engine)
  let db = null

  if (typeof engine === 'undefined') {
    console.warn('未传入到 数据库引擎 环境配置，将跳过数据库初始化。')
  } else if (typeof engine !== 'string') {
    console.error('未传入有效的数据库引擎名称', engine)
  } else if (!engines.includes(engine)) {
    console.warn(`对于数据库 '${engine}'，当前应用没有声明相应的处理逻辑。`)
  } else {
    // 将各数据库driver/ORM封装入引擎，不对外暴露
    switch (engine) {
      case 'RDB': // RDB - MySQL w/ Prisma
        db = await RDB.connect()
        break

      case 'DDB': // DDB - MongoDB
        db = await DDB.connect()
        break

      default: // 兜底容错
        console.warn(`对于数据库 '${engine}'，当前应用没有声明相应的处理逻辑；数据库相关的业务（若有）会遇到问题。`)
    }
  }
 ...
 *
 * @param value {string}
 * @param handler 对value的处理方法
 * @returns
 */
const coverPassword = (
  value: string,
  handler = value => value?.length > 0 ? 'set' : 'unset'
): null | string => {
